    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $feedback->phone ?? 'N/A' }}</div>
                                        <div class="text-sm text-gray-500">{{ $feedback->week ?? 'N/A' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    @php
                                        $good = $feedback->good ?? '';
                                        echo strlen($good) > 50 ? substr($good, 0, 50) . '...' : $good;
                                    @endphp
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    @php
                                        $bad = $feedback->bad ?? '';
                                        echo strlen($bad) > 50 ? substr($bad, 0, 50) . '...' : $bad;
                                    @endphp
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if($feedback->referrer)
                                    <span class="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium">{{ $feedback->referrer }}</span>
                                @else
                                    <span class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs font-medium">No Referrer</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    @if($feedback->week)
                                        @php
                                            // Parse custom week format (2025-W01, 2025-W02, etc.)
                                            if (preg_match('/2025-W(\d+)/', $feedback->week, $matches)) {
                                                $weekNum = (int) $matches[1];
                                                $baseDate = \Carbon\Carbon::create(2025, 6, 23); // June 23, 2025 is Week 1
                                                $weekStart = $baseDate->copy()->addWeeks($weekNum - 1);
                                                $weekEnd = $weekStart->copy()->addDays(6);
                                                echo "<div>Week {$weekNum}</div>";
                                                echo "<div class=\"text-xs text-gray-500\">{$weekStart->format('M d')} - {$weekEnd->format('M d')}</div>";
                                            } else {
                                                echo "<div>{$feedback->week}</div>";
                                            }
                                        @endphp
                                    @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    {{ $feedback->created_at ? $feedback->created_at->format('M d, Y') : 'N/A' }}
                                </div>
                                <div class="text-sm text-gray-500">
                                    {{ $feedback->created_at ? $feedback->created_at->format('h:i A') : 'N/A' }}
                                </div>
                            </td>
                        </tr>
                        @empty
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Selected Week</p>
                        <p class="text-2xl font-bold text-gray-900">
                            @php
                                $selectedWeekCount = App\Models\Feedback::where('week', $selectedWeek)->count();
                                echo $selectedWeekCount;
                            @endphp
                        </p>
                    </div>
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">
                            @php
                                $withReferrerCount = App\Models\Feedback::whereNotNull('referrer')->count();
                                echo $withReferrerCount;
                            @endphp
                        </p>
                    </div>
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">
                            @php
                                $uniquePhonesCount = App\Models\Feedback::distinct()->count('phone');
                                echo $uniquePhonesCount;
                            @endphp
                        </p>
                    </div>
